...
export class YearList implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'is_active', label: 'Attivo', type: 'boolean', sortable: true },
  ...
  actions: TableAction[] = [
    ...
    },
    {
      ...
      icon: 'star',
      color: 'warn',
      action: (year: Year) => this.setCurrentYear(year),
      visible: (year: Year) => !year.is_active
    },
    {
      label: 'Elimina',
      icon: 'delete',
  ...
  onFilterChange() {
    ...
    this.activeFilter = this.filters.is_active ?? undefined;
